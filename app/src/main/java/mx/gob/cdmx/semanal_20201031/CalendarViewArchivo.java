...
// builder.setMessage(cuentaArchivos())
	builder.setMessage(
		...
		"Rechazo por temor a contagio:\t\t\t" + dameRechazoContagio(fecha)+"\n\n"+
...
// Recorremos el cursor hasta que no haya más registros
	...
	public String dameRechazoContagio(String fecha) {
		String DATABASE_NAME = Environment.getExternalStorageDirectory() +"/Mis_archivos/" +nombreEncuesta+"_"+sacaImei()+"";
...
public String dameFiltro(String fecha) {
	String DATABASE_NAME = Environment.getExternalStorageDirectory() +"/Mis_archivos/" +nombreEncuesta+"_"+sacaImei()+"";
...
// Recorremos el cursor hasta que no haya más registros
		c.moveToFirst();
		cuantosFiltros = c.getString(0);
	} else {
		cuantosFiltros = "0";
	}
	c.close();
	db.close();
	return cuantosFiltros;
}
